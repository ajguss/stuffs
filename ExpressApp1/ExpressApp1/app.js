        ...
        req.session.user = new User(req.user, function()
        {
			...
			request(req.session.user.inv, function(error, response, body) {
				...
				for (var i in rgdesc){
					...
					urls[c] = "http://cdn.steamcommunity.com/economy/image/" + rgdesc[i].icon_url;
					names[c] = rgdesc[i].market_name;
					c++;
